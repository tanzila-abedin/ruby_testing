...
describe Dog do
  ...
  subject(:skylar) {described_class.new('skylar', 'great dane')}
  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'when dog shares the same method with multiple classes' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(skylar).to_not be_barking
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(skylar).to be_sleeping
  end
end
